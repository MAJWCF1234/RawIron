...
struct PostProcessState {
    ...
    float barrelDistortion = 0.0f;
    float chromaticAberration = 0.0f;
    /// Native Vulkan HDR composite (mirrors `ri::render::PostProcessParameters`).
    float casSharpenAmount = 0.0f;
    float casContrastAdaptation = 0.0f;
    float bloomIntensity = 0.0f;
...
/// Native Vulkan presentation can later merge `ri::render::ShaderPresentationConfig` from
/// `ResolveShaderCfgPath(environment.contentRoot, executableDirectory)` into the frame post chain
/// (same helper as ParticleShowcase).
